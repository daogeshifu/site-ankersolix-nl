        ...
        <div class="lg:col-span-8 space-y-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div class="flex flex-col gap-4 group cursor-pointer">
                    <a href="{{ route('article.detail.show', [$article->category->name ?? 'blog', $article->link]) }}" class="relative overflow-hidden rounded-xl aspect-video">
                        <img class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" src="{{ $article->cover ? Storage::url($article->cover) : '/around/picture/0126.jpg' }}" alt="{{ $article->title }}" loading="lazy">
                        @if($article->category)
            ...
            <div class="space-y-6 pt-6">
                ...
                <a href="{{ route('article.detail.show', [$sidebarArticle->category->name ?? 'blog', $sidebarArticle->link]) }}" class="flex gap-6 items-start border-t border-gray-100 dark:border-gray-800 pt-6 group cursor-pointer">
                    <img class="w-32 h-24 rounded-lg object-cover flex-shrink-0" src="{{ $sidebarArticle->cover ? Storage::url($sidebarArticle->cover) : '/around/picture/0127.jpg' }}" alt="{{ $sidebarArticle->title }}" loading="lazy">
                    <div class="space-y-1">
...
{{--            @endif--}}
            ...
            <div class="space-y-6">
                ...
                <a href="{{ route('article.detail.show', [$popArticle->category->name ?? 'blog', $popArticle->link]) }}" class="flex items-start gap-4 group">
                    <img class="size-16 rounded-lg object-cover flex-shrink-0" src="{{ $popArticle->cover ? Storage::url($popArticle->cover) : '/around/picture/0127.jpg' }}" alt="{{ $popArticle->title }}" loading="lazy">
                    <div class="space-y-1">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                ...
                <article class="bg-white dark:bg-[#0c121d] rounded-xl overflow-hidden shadow-sm hover:shadow-lg transition-shadow group">
                    <a href="{{ route('article.detail.show', [$latestArticle->category->name ?? 'blog', $latestArticle->link]) }}" class="block aspect-video overflow-hidden">
                        <img class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" src="{{ $latestArticle->cover ? Storage::url($latestArticle->cover) : '/around/picture/0126.jpg' }}" alt="{{ $latestArticle->title }}" loading="lazy">
                    </a>
                    <div class="p-6">
