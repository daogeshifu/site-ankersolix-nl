...
<main class="max-w-[1280px] mx-auto px-6 md:px-20 py-8">
    ...
    <nav class="flex items-center gap-2 mb-6 text-sm">
        ...
        <a class="text-[#616f89] hover:text-primary" href="{{ route('cases') }}">cases</a>
        <span class="text-[#616f89]">/</span>
    ...
    <div class="flex flex-col lg:flex-row gap-12">
        ...
        <div class="flex-[2] flex flex-col gap-8">
            ...
            <article class="group flex flex-col md:flex-row items-stretch gap-6 bg-white dark:bg-[#1e293b] rounded-xl p-4 shadow-sm hover:shadow-md transition-shadow border border-[#f0f2f4] dark:border-[#334155]">
                <a href="{{ route('article.detail.show', [$topArticle->category->name ?? 'blog', $topArticle->link]) }}" class="w-full md:w-72 h-48 bg-cover bg-center rounded-lg flex-shrink-0 block" style="background-image: url('{{ $topArticle->cover_url ?: '/around/picture/0126.jpg' }}');"></a>
            ...
            <article class="group flex flex-col md:flex-row items-stretch gap-6 bg-white dark:bg-[#1e293b] rounded-xl p-4 shadow-sm hover:shadow-md transition-shadow border border-[#f0f2f4] dark:border-[#334155]">
                <a href="{{ route('article.detail.show', [$article->category->name ?? 'blog', $article->link]) }}" class="w-full md:w-72 h-48 bg-cover bg-center rounded-lg flex-shrink-0 block" style="background-image: url('{{ $article->cover_url ?: '/around/picture/0126.jpg' }}');"></a>
                <div class="flex flex-col justify-between py-1 flex-1">
                    <div class="flex flex-col gap-3">
                        <div class="flex items-center gap-3">
                            @if($article->category)
