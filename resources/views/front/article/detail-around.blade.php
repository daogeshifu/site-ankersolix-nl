...
@section('content')
    ...
    <nav aria-label="breadcrumb">
      <ol class="pt-lg-3 pb-lg-4 pb-2 breadcrumb">
        ...
        <li class="breadcrumb-item active" aria-current="page">
          {{ $article->category->name ?? __('article.uncategorized') }}</li>
      </ol>
    </nav>
    ...
    <div class="row">
      ...
      <div class="col-lg-8 pb-2 pb-lg-0 mb-4 mb-lg-0">
        ...
        <div class="d-flex flex-wrap align-items-center mt-n2">
          ...
          <span class="fs-xs opacity-20 mt-2 mx-3">|</span>
          @if($article->category)
            <a class="badge text-nav fs-xs border mt-2"
              href="{{ route('article.category', $article->category->name) }}">{{ $article->category->name }}</a>
          @endif
        </div>
      </div>
      ...
      <div class="col-lg-4 col-xl-3 offset-xl-1">
        <div class="card border-0 bg-secondary">
          <div class="card-body text-center">
            <img class="d-block rounded-circle mx-auto mb-3" src="/around/picture/0332.jpg" width="80"
              alt="{{ __('article.author_image') }}">
  ...
  @if($article->cover)
    <section class="container mb-4">
      <img class="w-100 rounded-5" src="{{ Storage::url($article->cover) }}" alt="{{ $article->title }}"
        ...
        <div class="position-sticky top-0 mt-2 mt-md-3 mt-lg-0" style="padding-top: 7rem;">
          ...
          <div class="d-flex mt-n3 ms-n3 mb-lg-5 mb-4 pb-3 pb-lg-0">
            <a class="btn btn-outline-secondary btn-icon btn-sm btn-facebook rounded-circle mt-3 ms-3"
              ...
              aria-label="{{ __('article.share_on_facebook') }}">
              <i class="ai-facebook"></i>
            </a>
            <a class="btn btn-outline-secondary btn-icon btn-sm btn-x rounded-circle mt-3 ms-3"
              href="https://twitter.com/intent/tweet?url={{ urlencode(url()->current()) }}&text={{ urlencode($article->title) }}"
              target="_blank" aria-label="{{ __('article.share_on_x') }}">
              <i class="ai-x"></i>
            </a>
            <a class="btn btn-outline-secondary btn-icon btn-sm btn-linkedin rounded-circle mt-3 ms-3"
              href="https://www.linkedin.com/sharing/share-offsite/?url={{ urlencode(url()->current()) }}" target="_blank"
              aria-label="{{ __('article.share_on_linkedin') }}">
            ...
            <ul class="list-unstyled mb-0">
              @foreach($sidebarArticles as $sidebarArticle)
                <li class="{{ !$loop->last ? 'border-bottom pb-3 mb-3' : '' }}">
                  <span class="h6 mb-0">
                    <a
                      href="{{ route('article.detail.show', [$sidebarArticle->category->name ?? 'blog', $sidebarArticle->link]) }}">{{ $sidebarArticle->title }}</a>
                  </span>
                </li>
              @endforeach
            </ul>
    ...
    <section class="container pt-2 pt-sm-3 pb-5 mb-md-3 mb-lg-4 mb-xl-5">
      <div class="d-flex align-items-center pb-3 mb-3 mb-lg-4">
        ...
        <div class="d-flex ms-auto">
          <button class="btn btn-prev btn-icon btn-sm btn-outline-primary rounded-circle me-3" type="button" id="prev-post"
            aria-label="Prev">
            ...
            <article class="swiper-slide">
              <div class="position-relative">
                @if($relatedArticle->cover)
                  <img class="rounded-5" src="{{ Storage::url($relatedArticle->cover) }}" alt="{{ $relatedArticle->title }}"
                    style="width: 100%; height: 200px; object-fit: cover;">
                @else
                  <img class="rounded-5" src="/around/picture/0133.jpg" alt="{{ $relatedArticle->title }}"
                    style="width: 100%; height: 200px; object-fit: cover;">
                @endif
                <h3 class="h4 mt-4 mb-0">
                  <a class="stretched-link"
                    href="{{ route('article.detail.show', [$relatedArticle->category->name ?? 'blog', $relatedArticle->link]) }}">{{ $relatedArticle->title }}</a>
                </h3>
              </div>
            </article>
          @endforeach
