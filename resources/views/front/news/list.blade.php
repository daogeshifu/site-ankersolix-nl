...
@section('title', __('lang.seo_news_title'))
@section('description', __('lang.seo_news_description'))
@section('keywords', __('lang.seo_news_keywords'))
...
@section('opengraph')
    ...
    <meta property="og:title" content="{{ __('lang.seo_news_title') }}">
    <meta property="og:description" content="{{ __('lang.seo_news_description') }}">

    <meta name="twitter:card" content="summary_large_image">
    <meta name="twitter:title" content="{{ __('lang.seo_news_title') }}">
    <meta name="twitter:description" content="{{ __('lang.seo_news_description') }}">
@endsection
    ...
    <div class="flex flex-col lg:flex-row gap-12">
        ...
        <div class="flex-[2] flex flex-col gap-8">
            ...
            <div class="bg-primary/10 border border-primary/20 rounded-xl p-4 flex items-center gap-3">
                ...
                <a href="{{ route('news') }}" class="text-primary font-medium text-sm hover:underline">{{ __('article.clear_search') }}</a>
            </div>
            @endif
            ...
            <article class="group flex flex-col md:flex-row items-stretch gap-6 bg-white dark:bg-[#1e293b] rounded-xl p-4 shadow-sm hover:shadow-md transition-shadow border border-[#f0f2f4] dark:border-[#334155]">
                <a href="{{ route('news.detail.show', $topArticle->link) }}" class="w-full md:w-72 h-48 bg-cover bg-center rounded-lg flex-shrink-0 block" style="background-image: url('{{ $topArticle->cover ? Storage::url($topArticle->cover) : '/around/picture/0126.jpg' }}');"></a>
                <div class="flex flex-col justify-between py-1 flex-1">
                    <div class="flex flex-col gap-3">
                        ...
                        <h2 class="text-2xl font-bold group-hover:text-primary transition-colors leading-tight">
                            <a href="{{ route('news.detail.show', $topArticle->link) }}">{{ $topArticle->title }}</a>
                        </h2>
            ...
            <article class="group flex flex-col md:flex-row items-stretch gap-6 bg-white dark:bg-[#1e293b] rounded-xl p-4 shadow-sm hover:shadow-md transition-shadow border border-[#f0f2f4] dark:border-[#334155]">
                <a href="{{ route('news.detail.show', $article->link) }}" class="w-full md:w-72 h-48 bg-cover bg-center rounded-lg flex-shrink-0 block" style="background-image: url('{{ $article->cover ? Storage::url($article->cover) : '/around/picture/0126.jpg' }}');"></a>
                <div class="flex flex-col justify-between py-1 flex-1">
                    <div class="flex flex-col gap-3">
                        <div class="flex items-center gap-3">
                            @if($article->category)
                        ...
                        <h3 class="text-xl md:text-2xl font-bold group-hover:text-primary transition-colors leading-tight">
                            <a href="{{ route('news.detail.show', $article->link) }}">{{ $article->title }}</a>
                        </h3>
                        <p class="text-[#616f89] dark:text-[#94a3b8] text-sm line-clamp-3">
            ...
            <div class="flex items-center justify-center gap-2 mt-8 py-6">
                ...
                @else
                <a href="{{ route('news.page', $articles->currentPage() - 1) }}" class="size-10 flex items-center justify-center rounded-lg border border-[#f0f2f4] dark:border-[#334155] hover:bg-white dark:hover:bg-[#1e293b] transition-colors">
                    <span class="material-symbols-outlined">chevron_left</span>
                </a>
                @endif
                ...
                @if($start > 1)
                <a href="{{ route('news') }}" class="size-10 flex items-center justify-center rounded-lg hover:bg-white dark:hover:bg-[#1e293b] transition-colors font-medium">1</a>
                    @if($start > 2)
                ...
                @for($i = $start; $i <= $end; $i++)
                    ...
                    @else
                    <a href="{{ $i == 1 ? route('news') : route('news.page', $i) }}" class="size-10 flex items-center justify-center rounded-lg hover:bg-white dark:hover:bg-[#1e293b] transition-colors font-medium">{{ $i }}</a>
                    @endif
                @endfor
                ...
                @if($end < $lastPage)
                    ...
                    <span class="px-2 text-[#616f89]">...</span>
                    @endif
                <a href="{{ route('news.page', $lastPage) }}" class="size-10 flex items-center justify-center rounded-lg hover:bg-white dark:hover:bg-[#1e293b] transition-colors font-medium">{{ $lastPage }}</a>
                @endif
